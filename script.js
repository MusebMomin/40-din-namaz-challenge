...
function setupEventListeners() {
  ...
  if (salahForm) {
    ...
    // Live reward preview: fires on any checkbox/select change inside the form
    salahForm.addEventListener('change', checkRewardPreview);
  }
...
function checkRewardPreview() {
  const allTakbeer = PRAYERS.every((p) => {
    const jamat  = document.querySelector(`.status-checkbox[data-prayer="${p}"][data-status="jamat"]`);
    const takbeer = document.querySelector(`.takbeer-checkbox[data-prayer="${p}"]`);
    return jamat?.checked && takbeer?.checked;
  });

  const allSunnat = PRAYERS.every((p) => {
    const sel = document.querySelector(`.sunnat-select[data-prayer="${p}"]`);
    return (sel ? Number(sel.value) : 0) >= SUNNAT_MAX[p];
  });

  if (allTakbeer && allSunnat) showRewardPreview();
}

function showRewardPreview() {
  const existing = document.getElementById('reward-preview-badge');
  if (existing) { existing.remove(); clearTimeout(_rewardTimer); }

  const badge = document.createElement('div');
  badge.id = 'reward-preview-badge';
  badge.className = 'reward-preview';
  badge.innerHTML = '🏆 Perfect Day!<br><small style="font-size:0.82rem;opacity:.9">+2 Streak &amp; +1 Lifeline on submit</small>';
  document.body.appendChild(badge);

  _rewardTimer = setTimeout(() => {
